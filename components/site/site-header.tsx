  ...
  return (
    <header className="sticky top-0 z-50 border-b border-neutral-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:h-[72px] lg:px-8">
        <Link className="group flex items-center gap-3" href="/" aria-label="Tuff Workwear home" onClick={() => setMenuOpen(false)}>
          <svg viewBox="-5 -10 110 102" aria-hidden="true" className="h-10 w-10 shrink-0">
      ...
      {menuOpen && (
        <nav
          className="border-t border-white/10 bg-[#2D2D2D] lg:hidden"
        ...
        >
          <ul className="flex flex-col">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block px-6 py-4 text-base font-medium text-white transition-colors hover:text-brand-orange"
          ...
          <div className="border-t border-white/10 px-6 py-4">
            <Button asChild size="sm" variant="whatsapp" className="w-full">
              <a href={heroWhatsappHref} onClick={() => setMenuOpen(false)}>
                WhatsApp us
              </a>
            </Button>
          </div>
